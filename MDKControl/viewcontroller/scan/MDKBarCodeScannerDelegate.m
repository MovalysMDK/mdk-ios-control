...
@interface MDKBarCodeScannerDelegate ()

/**
 * @brief The source object that uses this delegate
 */
@property (nonatomic, weak) id<MDKBarCodeScannerProtocol> sourceScanner;

@end

@implementation MDKBarCodeScannerDelegate

#pragma mark - Initializing

-(id) initWithSource:(id<MDKBarCodeScannerProtocol>)source {
    self = [super init];
    if(self) {
...
-(void)postInitialize {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self.sourceScanner selector: @selector(orientationDidChanged:) name: UIDeviceOrientationDidChangeNotification object:nil];
}
