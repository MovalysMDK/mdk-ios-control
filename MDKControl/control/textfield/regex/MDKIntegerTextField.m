...
@interface MDKIntegerTextField ()

@property (nonatomic, strong) UIBarButtonItem *minusButton;

@end

@implementation MDKIntegerTextField
...
-(void) signButtonClick:(UIBarButtonItem *)sender {
    if([sender.title isEqualToString:@"—"]) {
        [self setData:[NSString stringWithFormat:@"-%@", [self stringData]]];
        sender.title = @"+";
    }
    else {
        if([[self stringData] rangeOfString:@"-"].location != NSNotFound) {
            [self setData:[[self stringData] substringFromIndex:1]];
        }
        sender.title = @"—";
    }
...
-(NSArray *)controlValidators {
    return @[[MDKIntegerFieldValidator sharedInstance]];
}
